...
from calcs import slope_corrected, vapp_corrections, wind_correct_formulated, max_landing_wt_lda, get_torque_limits
...
xls = pd.ExcelFile('Q200 NTOP MTOP Cases.xlsx')
...
def all_data(all_row_data):
    ...
    ntop, mtop = get_torque_limits(temp, pressure_altitude, vapp)
...
def write_to_excel(all_exc):
    ...
    writer = pd.ExcelWriter('Tests_run_200.xlsx')
    ...
    df = df.style.applymap(lambda x: 'background-color: red' if '*' in str(x) else ('background-color: orange' if
                                                                                    '^' in str(x) else ''))
    df.to_excel(writer, sheet_name='Completed Tests 200', index=False)

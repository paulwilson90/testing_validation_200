...
def wind_correct_formulated(ULD, wind_comp, flap):
    """
    Flap 15, every m above 450 ULD, take off 0.002m (0.3 change over 150) on top of the base 2.7 for every knot head

    Flap 15, for every m above 450 ULD, add 0.008m (1.2 change over 150) on top of the base 8.6 for every knot tail

    flap 35 for every m above 420 ULD,(0.1 diff over 130) means take 0.00076923m on top of 2.7 base for
    every knot of head

    flap 35 for every m above 420 ULD, (0.9 diff over 130) means add 0.00692308m on top base 8.8 for
    any over 420 for tail"""
    flap = str(flap)
    ...
    if wind_comp > 0:  # flap 15 headwind
        ...
        if flap == "15":
            ...
            wind_corr_ULD = ULD - (wind_comp * (8.6 + factor_above_uld))
...
def get_torque_limits(temp, pressure_alt, vapp):
    if temp < 0:
        temp = 0
    if temp > 48:
        temp = 48
